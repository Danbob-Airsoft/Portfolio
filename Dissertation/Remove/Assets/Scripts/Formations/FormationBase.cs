...
public class FormationBase : MonoBehaviour
{
    ...
    private FriendlyMarker ThisMarkerScript;
    ...
    private void Start()
    {
        ...
        ThisMarkerScript = this.GetComponent<FriendlyMarker>();
    ...
    private void GenerateFormation()
    {
        ...
        this.transform.position = LeadTransform.position;
        this.transform.rotation = LeadTransform.rotation;
        ...
        for (int i = 0; i < UnitTotalCount; i++)
        {
            Vector3 NewSlotPosition = new Vector3();
            if(i == 0)
            {
                NewSlotPosition = this.transform.position;
            }
            ...
            else if (i < 2)
            {
                ...
                if (FlippedGate)
                {
                    NewSlotPosition = this.transform.position - SlotOffset;
                }
            ...
            GameObject SpawnedFighter = Instantiate(UnitPrefab, NewSlotPosition, this.transform.rotation);
            //Give fighter reference to this formation
            SpawnedFighter.GetComponentInChildren<FighterHealth>().ThisFormationBase = this;
            SpawnedFighter.GetComponentInChildren<FighterHealth>().FormationIndex = i;
            //Add Fighter to list
    ...
    public void RemoveFighter(GameObject ToRemove, int InIndex)
    {
        ...
        else
        {
            for(int i = 0; i < ActiveUnitCount; i++)
            {
                ...
                //Active ships individual markers
    ...
    public void EnterCombat()
    {
        ...
        ThisMarkerScript.MarkerActive = false;
    }
    ...
    public void UpdateFormation()
    {
        ...
        if (InCombat)
        {
            ...
            if(Time.time > ReformTime)
            {
                ...
                InCombat = false;
                //Enable formation marker script
                ThisMarkerScript.MarkerActive = true;
    ...
    private void RandomMovement()
    {
        ...
        if(Time.time >= NextChangeTime || Vector3.Distance(this.transform.position, RandomTarget) <= 10)
        {
            //Update Change Time
        ...
        Vector3 VectToTarget = RandomTarget - this.transform.position;
        if (Vector3.Angle(this.transform.forward, VectToTarget) != 0)
        {
    ...
    private void FormationSpecific()
    {
        ...
        ThisBody.AddForce(this.transform.forward * (MoveSpeed - 1), ForceMode.Impulse);
    ...
    {
        GameObject FighterOBJ = Fighter.FighterOBJ;
        Quaternion ToSlot = Quaternion.LookRotation(ShipSlot + (this.transform.forward * 5) - FighterOBJ.transform.position, this.transform.up);
        //Check if arrived at rotation
    ...
    {
        Quaternion PlayerRotation = Quaternion.LookRotation(PlayerOBJ.transform.position - this.transform.position);
        //Turn Formation towards Player
        this.transform.rotation = Quaternion.RotateTowards(this.transform.rotation, PlayerRotation, RotationSpeed * Time.deltaTime);
    }
    ...
    private void UpdateSlotPositions()
    {
        ...
        if(SlotPositions.Count != 0)
        {
            ...
            SlotPositions[0] = this.transform.position;
            ...
            if (FlippedGate)
            {
                ...
                for (int i = 1; i < SlotPositions.Count; i++)
                {
                    ...
                    SlotPositions[i] = RotatePointAroundPivot(SlotHolder, this.transform.position, this.transform.eulerAngles);
                }
            }
            else
            {
    ...
    private void MoveUnitsToSlots()
    {
        for(int i = 0; i < ActiveUnitCount; i++)
        {
            ...
            if (CurrentShipStruct.Avoiding == false && CurrentShipStruct.ReturningToSlot == false)
            {
                ...
                q = Quaternion.LookRotation(TargetSlot + (this.transform.forward * 5) - Ship.transform.position, this.transform.up);
