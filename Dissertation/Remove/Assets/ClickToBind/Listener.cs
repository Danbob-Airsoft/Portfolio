...
public class Listener : MonoBehaviour {
	...
	void Update () {
		...
		if(KeyBindingManager.GetKeyDown(KeyAction.Accelerate))
		{
			box1.SetActive(!box1.activeSelf);
		}

		if(KeyBindingManager.GetKeyDown(KeyAction.RollLeft))
		{
			box3.SetActive(!box3.activeSelf);
		}
				
		if(KeyBindingManager.GetKeyDown(KeyAction.RollRight))
		{
