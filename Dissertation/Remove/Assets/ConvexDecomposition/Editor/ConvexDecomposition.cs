    ...
    public class ConvexDecomposition : MonoBehaviour
    {
            ...
            PhysicMaterial physicMaterial, bool createAsset, bool createColliders, bool createRenderers)
        {
            ...
            if (createAsset)
            {
                ...
                {
                    Debug.Log("Mesh colliders asset path not selected, canceling");
                    return;
                }
            }
            ...
            createRenderers = createRenderers && parentShader;
            ...
            try
            {
                ...
                for (int i = 0; i < convexMeshes.Count; i++)
                {
                    ...
                    convexMesh.name = MeshName(gameObject, i);

                    GameObject go = new GameObject(ChildObjectName(gameObject, i));
            ...
            if (createAsset)
            {
                Object[] meshObjects = AssetDatabase.LoadAllAssetsAtPath(assetPath);
                Mesh[] meshes = meshObjects.Cast<Mesh>().ToArray();
                
                for (int i = 0; i < convexMeshes.Count && i < meshes.Length; i++)
                {
                    GameObject go = bakedColliders_T.Find(ChildObjectName(gameObject, i)).gameObject;
                    Mesh mesh = meshes[i];
        ...
        private static string ChildObjectName(GameObject parent, int i)
        {
            return string.Format("{0} collider{1:000}", parent.name, i);
        }

        private static string MeshName(GameObject parent, int i)
        {
            return string.Format("{0} collider-mesh{1:000}", parent.name, i);
        }
    }
}
